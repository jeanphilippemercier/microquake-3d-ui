...
class FlinnEngdahl(object):
    ...
    def __init__(self):
        ...
        with open(self.names_file, 'r', encoding="utf-8") as fh:
            self.names = [name.strip() for name in fh]

        with open(self.quadsindex_file, 'r', encoding="utf-8") as fh:
            indexes = []
            for index in fh:
        ...
        for quad, sect_file in zip(self.quads_order, self.sect_files):
            sect = []
            with open(sect_file, 'r', encoding="utf-8") as fh:
                for line in fh:
        ...
        with open(self.numbers_file, 'rt', encoding="utf-8") as csvfile:
